    ...
    <div className="flex items-center gap-2">
      <div
        className={[
          ...
          done ? "bg-green-600 text-white border-green-600" : active ? "bg-[#008000] text-white border-[#008000]" : "bg-gray-200 text-gray-500 border-gray-200",
        ].join(" ")}
      >
        {done ? "✓" : idx}
      </div>
      <div className={done || active ? "text-xs font-semibold text-gray-800" : "text-xs font-medium text-gray-400"}>
        {idx === 1 ? "Select College" : idx === 2 ? "Select Course" : idx === 3 ? "Fill Details" : "Confirmation"}
      </div>
    </div>
  );
}
...
export default function UserDashboard({ onLogout, onNavigate }) {
  ...
  const selectedCollege = useMemo(() => COLLEGES.find((c) => c.key === selectedCollegeKey) || null, [selectedCollegeKey]);
  ...
  const viewCollege = useMemo(() => COLLEGES.find((c) => c.key === viewCollegeKey) || null, [viewCollegeKey]);

  const canGoNextFromStep1 = !!selectedCollege;
  const canGoNextFromStep2 = !!selectedCourse;
  const canGoNextFromStep3 = details.fullName.trim() && details.studentId.trim();
    ...
    <div className="flex min-h-screen bg-gray-50">
      <aside className="w-64 shrink-0 bg-[#008000] text-white flex flex-col">
        ...
        <div className="p-4 border-t border-green-600/50">
          <p className="text-sm font-medium">{new Date().toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</p>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-[#008000]">SELECT DEPARTMENT</h1>
            <p className="text-xs text-gray-500">Choose Your College To Log Your Attendance</p>
          </div>
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg px-6 py-4 mb-6 flex flex-wrap gap-4 text-sm">
            <span>
              <strong>EVENT:</strong> {EVENT_CARD.eventName}
            </span>
            <span>
              <strong>AY:</strong> {EVENT_CARD.ay}
            </span>
            <span>
              <strong>VENUE:</strong> {EVENT_CARD.venue}
            </span>
          </div>
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        selected ? "border-[#008000] ring-2 ring-blue-400/70 shadow-sm" : "border-gray-200 hover:border-[#008000]/60",
                      ].join(" ")}
                    >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-full bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center">
                            {c.logoSrc ? (
                              ...
                              <span className="text-xs font-semibold text-gray-800">{c.iconText}</span>
                            )}
                          </div>
                          <div>
                            <div className="text-sm font-semibold text-gray-900">{c.key}</div>
                      ...
                      <div className="mt-3 flex items-end justify-between">
                        <span className="text-xs text-gray-600">{c.footerBadge}</span>
                        <span
                          role="button"
                          tabIndex={0}
          ...
          {step === 2 && selectedCollege && (
            <>
              <div className="flex items-center justify-between gap-3 mb-3">
                <h2 className="text-sm font-semibold text-gray-800">Select Course</h2>
              </div>
                ...
                {selectedCollege.courses.map((course) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[11px] text-gray-500">{selectedCollege.key}</div>
                    </button>
                  );
                })}
              </div>
          ...
          {step === 3 && selectedCollege && selectedCourse && (
            <>
              <div className="flex items-center justify-between gap-3 mb-3">
                <h2 className="text-sm font-semibold text-gray-800">Fill Details</h2>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <label className="block text-xs font-medium text-gray-600">
                  ...
                  <select
                    ...
                    onChange={(e) => setDetails((d) => ({ ...d, yearLevel: e.target.value }))}
                ...
                <label className="block text-xs font-medium text-gray-600">
                  ...
                  <input
                    ...
                    onChange={(e) => setDetails((d) => ({ ...d, section: e.target.value }))}
                    className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#008000] focus:border-[#008000]"
                    placeholder="e.g., A"
                  />
                </label>
                <label className="block text-xs font-medium text-gray-600">
                  ...
                  <input
                    ...
                    onChange={(e) => setDetails((d) => ({ ...d, timeIn: e.target.value }))}
                    className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#008000] focus:border-[#008000] bg-white"
                  />
                </label>
          ...
          {step === 4 && selectedCollege && selectedCourse && (
            <>
              <div className="flex items-center justify-between gap-3 mb-3">
                <h2 className="text-sm font-semibold text-gray-800">Confirmation</h2>
              </div>

              <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-5 space-y-3">
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-gray-600">Event</span>
                  <span className="font-semibold text-gray-900">{EVENT_CARD.eventName}</span>
                </div>
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-gray-600">College</span>
                  <span className="font-semibold text-gray-900">{selectedCollege.key}</span>
                </div>
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-gray-600">Course</span>
                  <span className="font-semibold text-gray-900">{selectedCourse.label}</span>
                </div>
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-gray-600">Full Name</span>
                  <span className="font-semibold text-gray-900">{details.fullName}</span>
                </div>
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-gray-600">Student ID</span>
                  <span className="font-semibold text-gray-900">{details.studentId}</span>
                </div>
                {details.yearLevel && (
                  <div className="flex items-center justify-between gap-3 text-sm">
                    <span className="text-gray-600">Year Level</span>
                    <span className="font-semibold text-gray-900">{details.yearLevel}</span>
                  </div>
                )}
                {details.section && (
                  <div className="flex items-center justify-between gap-3 text-sm">
                    <span className="text-gray-600">Section</span>
                    <span className="font-semibold text-gray-900">{details.section}</span>
                  </div>
                )}
                {details.timeIn && (
                  <div className="flex items-center justify-between gap-3 text-sm">
                    <span className="text-gray-600">Time In</span>
                    <span className="font-semibold text-gray-900">{details.timeIn}</span>
                  </div>
                )}
                {details.timeOut && (
                  <div className="flex items-center justify-between gap-3 text-sm">
                    <span className="text-gray-600">Time Out</span>
                    <span className="font-semibold text-gray-900">{details.timeOut}</span>
                  </div>
                )}
          ...
          {step < 4 && (
            <div className="mt-6 flex items-center justify-between gap-3">
              ...
              <button
                ...
                disabled={(step === 1 && !canGoNextFromStep1) || (step === 2 && !canGoNextFromStep2) || (step === 3 && !canGoNextFromStep3)}
              >
                Next
              </button>
            </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
            <div className="bg-[#008000] px-5 py-4 flex items-start justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h3 className="text-white font-semibold text-sm truncate">{viewCollege.title}</h3>
                  <p className="text-[11px] text-green-100">{viewCollege.key} - {viewCollege.footerBadge}</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-4 space-y-3">
              <div className="text-xs text-gray-500 flex items-center gap-2">
                <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-gray-100 text-gray-600">⌁</span>
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        selected ? "border-[#008000] ring-2 ring-[#008000]/20" : "border-gray-200 hover:border-[#008000]/50",
                      ].join(" ")}
                    >
                      <div
                        className={[
                          "mt-0.5 w-6 h-6 rounded-full flex items-center justify-center border text-xs font-semibold",
                          selected ? "bg-[#008000] text-white border-[#008000]" : "bg-green-50 text-[#008000] border-green-200",
                        ].join(" ")}
                      >
                        {i + 1}
                      </div>
                      <div className="min-w-0">
                        <div className="text-sm font-semibold text-gray-800">{course.label}</div>
                      ...
                      <div className="ml-auto mt-0.5">
                        <div
                          className={[
                            ...
                            selected ? "border-[#008000] bg-[#008000]" : "border-gray-300 bg-white",
                          ].join(" ")}
                        >
